...
if [ "$user" == "root" ]; then
	...
	sleep 2
	sudo apt install -y kitty rofi feh xclip ranger betterlockscreen brightessctl scrot scrub wmname imagemagick cmatrix htop neofetch python3-pip procps tty-clock fzf lsd bat pamixer flameshot playerctl bluez dunst gawk blueman
	if [ $? != 0 ] && [ $? != 130 ]; then
