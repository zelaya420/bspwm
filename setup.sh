...
if [ "$user" == "root" ]; then
	...
	sleep 2
	sudo apt install -y kitty rofi feh xclip ranger betterlockscreen brightnessctl scrot scrub jq wmname imagemagick cmatrix htop neofetch python3-pip procps tty-clock fzf lsd bat pamixer flameshot playerctl bluez dunst gawk blueman
	if [ $? != 0 ] && [ $? != 130 ]; then
	...
	sleep 2
	sudo apt install -y build-essential git redshift vim libxcb-util0-dev libxcb-ewmh-dev libxcb-randr0-dev libxcb-icccm4-dev libxcb-keysyms1-dev libxcb-xinerama0-dev libasound2-dev libxcb-xtest0-dev libxcb-shape0-dev libuv1-dev
	if [ $? != 0 ] && [ $? != 130 ]; then
